...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.startswith('/api/recordings/'):
            ...
            data=target.read_bytes(); self.send_response(200); self.send_header('Content-Type','application/vnd.tcpdump.pcap'); self.send_header('Content-Disposition',f'attachment; filename="{name}"'); self.send_header('Content-Length',str(len(data))); self.end_headers(); self.wfile.write(data); return
        super().do_GET()
    def do_POST(self):
        global proc
        p=urlparse(self.path).path
